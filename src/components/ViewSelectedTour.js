...
class ViewSelectedTour extends React.Component {
  ...
  sortLocations() {
    ...
    let locationsArray = this.props.tours.focusedTour
    locationsArray.sort( (a, b) => {
      let posA = parseInt(a.position)
      let posB = parseInt(b.position)
      return posA - posB
    })
    console.log(this.props.tours.focusedTour.locations, locationsArray)
  ...
  get controls() {
    ...
    if (this.state.displayMode === 'started') {
      ...
      return (
        ...
        {this.state.currentImage ? <p>Here's the photo you took!</p> :  <p>Once you've arrived at {this.state.viewLocs[this.state.locationCounter].name}, take an picture of it!</p>}
    ...
    let myWidget = window.cloudinary.createUploadWidget({
      ...
      uploadPreset: 'preset_test'}, (error, result) => {
        if (!error && result && result.event === "success") {
          ...
          urlArray.splice(urlArray.length-2, 1, "w_0.1,h_0.1,c_scale")
          let imageUrl = urlArray.join("/")

          let imageArray = this.state.images
          imageArray.push(imageUrl)
          this.setState({...this.state, currentImage: imageUrl, image: imageUrl})
        }
      })
